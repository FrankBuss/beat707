...
void createScreen()
{
  ...
  if (curLeftScreen == kLeftMain)
  {
    ...
    if (streamNextPattern || loadPatternNow) segments[0][7] |= 0B10000000;
    //
    ...
    if (showBPMdot) segments[0][5] |= 0B10000000;
  }
  //
  resetSegments(1, 2);
  //
    ...
    if (curTrack < DRUM_TRACKS)
    {
      for (byte xs=0; xs < 2; xs++)
      {
        ...
        {       
          ...
          //
          segments[xs + 1][x] = (char)stepChars[stepVelocity];
          //
          if (bitRead(stepsData[x + (xs*8)].stepsDouble[xVar], curTrack)) segments[xs + 1][x] |= 0B10000000;
        }
      }
    }
    else // NOTE TRACKS ---===----==---==-- //
    {
      if (editingNote)
      {
        ...
        else if (stepsData[editStep].noteSteps[curTrack-DRUM_TRACKS][editVariation] > 0 && stepVelocity != 0)
        {
          ...
          segments[2][6] = S_U;
          segments[2][7] = (char)numbers[stepVelocity];
          //
          if (xm == 1) 
          {
            segments[1][0] = S_G;
        ...
        {
          for (byte xs=0; xs < 8; xs++) { segments[2][xs + 8] = 0B01000000; }
        }
      }
      else // ----------------------------------------------------------------------------------- //
      {
        for (byte xs=0; xs < 2; xs++)
        {
          for (byte x=0; x < 8; x++)
          {
            ...
            if (xm != 3 && stepsData[x + (xs*8)].noteSteps[curTrack-DRUM_TRACKS][xVar] > 0 && stepVelocity != 0)
              segments[xs + 1][x] = (char)stepChars[stepVelocity];
            //
            if (xm == 1) segments[xs + 1][x] |= 0B01000000;
            else if (xm == 2) segments[xs + 1][x] |= 0B10000000;
            else if (xm == 3) segments[xs + 1][x] |= 0B01000000;            
          }
        }       
      }
    }
            ...
            if (editingNote)
            {
              ...
              else if (i == 1 && x == 6) // Octave
              {
                if (buttonEvent[i + 1][x] == kButtonClicked)
                {
                  ...
                  if (xNote <= 0) xNote = 12;
                  stepsData[editStep].noteSteps[curTrack-DRUM_TRACKS][editVariation] = byte(xNote & 0B01111111);
      ...
      curRightScreen == kRightMenu)
  {
    ...
    if (leButton >= 0)
    {
      ...
      else if (curRightScreen == kRightPatternSelection)
      {
        ...
        if (currentPattern != nextPattern || nextPatternBank != currentPatternBank) loadPattern(nextPattern);
      }
    }
  }
  //  
...
void checkIfMirrorAndCopy(byte thestep, byte track)
{
  if (mirror)
  {
    ...
    //
    stepsData[thestep].noteStepsExtras[track][0] &= 0B00000011;
    ...
    //
    stepsData[thestep].noteStepsExtras[track][1] &= 0B00000011;
...
{
  if (value <= 9) return (char)numbers[value];

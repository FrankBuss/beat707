  ...
  if (midi_available() > 0)
  {
    ...
    else if (midiInputStage == 0)
    {
      ...
      if (inputByte == 0xF8) // Midi Clock Sync Tick 24PPQ =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      {
        ...
        {
          if (configData.seqSyncOut) midi_write(0xF8);
          ledsBufferFlip();
      ...
      else if (inputByte == 0xFA) // MIDI Start =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      {
        if (!configData.midiClockInternal) 
        {
          if (configData.seqSyncOut) midi_write(0xFA);
      ...
      else if (inputByte == 0xFC) // MIDI Stop =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      {
        if (!configData.midiClockInternal) 
        {
          if (configData.seqSyncOut) midi_write(0xFC);
          stopSequencer();
        }
      }
      ...
      {
        if (configData.seqSyncOut && !configData.midiClockInternal && inputByte == 0xFB) midi_write(0xFB);
      }
      else
      {
        midiInputStage = 1;
      ...
      switch(midiInputBuffer[0] & 0xF0)
      {
        case 0x80: // Note Off -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        {
          ...
          //
            ...
            midi_write(midiInputBuffer[0]);
            midi_write(midiInputBuffer[1]);
            midi_write(lastData);
          #endif
          //
          break;
        ...
        case 0x90: // Note On =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        {
          ...
          //
            ...
            if (!recordEnabled && channel == configData.midiInputToPatternChannel)
            {
              ...
              {
                nextPattern = midiInputBuffer[1] - MIDI_IN_START_NOTE;
